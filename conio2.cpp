...
int cputs(const char *str) {
	...
	UpdateWScroll();
	if (WriteConsole(thData.output, str, strlen(str), &count, NULL)) return count;
	else return EOF;
};
...
int putch(int c) {
	DWORD count;
	UpdateWScroll();
	if (WriteConsole(thData.output, &c, 1, &count, NULL)) return c;
	else return EOF;
};
...
void settitle(const char *title) {
	//linia poni�ej generuje b��d w VS2013
